...
def test_tc12_method_not_allowed_on_consulta_get(client):
    response = client.get("/consulta")
    assert response.status_code == 405


def test_tc13_non_json_body(client):
    response = client.post("/consulta", data="no json body", content_type="text/plain")
    assert response.status_code == 415


def test_tc14_weather_synonym_pronostico(client):
...
def test_tc15_dollar_synonym_usd(client):
    response = client.post("/consulta", json={"consulta": "Consulta USD valor"})
    assert response.status_code == 200
    assert "dólar" in response.json["respuesta"].lower()


def test_tc16_news_synonym_titulares(client):
...
def test_tc17_fallback_unknown_intent(client):
