...
def test_process_instruction_no_intent():
...
def test_process_instruction_weather_intent(mock_weather):
...
def test_process_instruction_multiple_intents(mock_uf, mock_dollar):
...
def test_get_uf_response_success(monkeypatch):
...
def test_get_uf_response_failure(monkeypatch):
    def raise_error(url):
...
def test_get_dollar_response_success(monkeypatch):
...
def test_get_dollar_response_failure(monkeypatch):
    def raise_error(url):
...
def test_get_news_response_success(monkeypatch):
    articles = [
...
def test_get_news_response_no_articles(monkeypatch):
    fake_data = {"articles": []}
    fake_resp = FakeResponse(json_data=fake_data)
    monkeypatch.setattr(requests, 'get', lambda url: fake_resp)
...
def test_get_news_response_failure(monkeypatch):
    def raise_error(url):
...
def test_get_weather_response_all_success(monkeypatch):
    current = {
        "main": {"temp": 20, "humidity": 50},
        "weather": [{"description": "soleado"}],
        "wind": {"speed": 5}
    }
    yesterday = {"forecast": {"forecastday": [{"day": {"avgtemp_c": 18}}]}}
    tomorrow = {"forecast": {"forecastday": [{"day": {"avgtemp_c": 22}}]}}
    ...
    assert "Temperatura de ayer: 18" in resp
    assert "Temperatura pronosticada para mañana: 22" in resp


def test_get_weather_response_openweathermap_failure(monkeypatch):
    ...
    resp = main.get_weather_response()
    assert "No se pudo obtener el clima de Temuco" in resp
    assert "Temperatura de ayer: 18" in resp
    assert "Temperatura pronosticada para mañana: 22" in resp


def test_get_weather_response_yesterday_failure(monkeypatch):
    current = {
        "main": {"temp": 20, "humidity": 50},
    ...
    resp = main.get_weather_response()
    assert "Temperatura actual: 20" in resp
    assert "No se pudo obtener los datos históricos de ayer" in resp
    assert "Temperatura pronosticada para mañana: 22" in resp


def test_get_weather_response_tomorrow_failure(monkeypatch):
    current = {
        "main": {"temp": 20, "humidity": 50},
        "weather": [{"description": "lluvioso"}],
        "wind": {"speed": 4}
    }
    yesterday = {"forecast": {"forecastday": [{"day": {"avgtemp_c": 17}}]}}
    ...
    resp = main.get_weather_response()
    assert "Temperatura actual: 20" in resp
    assert "Temperatura de ayer: 17" in resp
